package com.example.feature_product_details.presentation.fragment
...
import com.example.utils.convertIntToPriceForProductDetails
...
class ProductDetailsFragment :
    BaseFragment<FragmentProductDetailsBinding, ProductDetailsDto, ProductDetailsViewModel>(
    ...
    override fun provideSuccess(data: ProductDetailsDto) {
        initViews(data)
        adapter?.setData(data.images)
    }

    @RequiresApi(Build.VERSION_CODES.M)
    private fun initViews(data: ProductDetailsDto) {
        with(viewBinding.bottomSheet) {
            ...
            val textButton = convertIntToPriceForProductDetails(data.price)
